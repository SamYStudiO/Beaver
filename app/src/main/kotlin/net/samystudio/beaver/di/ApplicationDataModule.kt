...
object ApplicationDataModule {
    ...
    fun providePreferencesDataStore(
        ...
        DataStoreFactory.create(
            serializer = PreferencesSerialize(servers),
    ...
    fun provideDatabase(@ApplicationContext context: Context): BeaverDatabase =
        Room.databaseBuilder(context, BeaverDatabase::class.java, DATABASE_FILE_NAME).build()
...
private class PreferencesSerialize(servers: List<Server>) : Serializer<Preferences> {
    override val defaultValue: Preferences =
        Preferences.newBuilder().setServer(servers.first()).build()
