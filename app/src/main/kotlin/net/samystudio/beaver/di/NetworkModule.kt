...
object NetworkModule {
            ...
            val newToken =
                if (actualToken == tokenRepository.get().data)
                    try {
                        ...
                        runBlocking { tokenRepository.get().refreshFromRemote() }
                    } catch (e: Exception) {
    ...
    fun provideRequestInterceptor(
        ...
        Interceptor { chain ->
            ...
            val server = runBlocking { preferencesManager.server.first() }.url

            // Let's set selected server url and acceptable languages header.
            newBuilder
                .url(
                    url.toString().replace(BASE_URL, server).toHttpUrl()
                )
                .header(
                    "Accept-Language",
            ...
            if (request.isAuthorizationRequired()) {
                try {
                    runBlocking { tokenRepository.get().getAsyncData() }.takeIf { it.isValid }
                        ?.let {
            ...
            try {
                ...
                crashlytics.log("RetrofitException url=${chain.request().url}, code=${e.code()}, message=${e.message()}")
